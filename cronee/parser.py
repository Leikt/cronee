...
def parse_value(value: str,
                valid_range: set,
                aliases: dict[str, set[int]] = None,
                inversion: bool = False) -> set[int]:
    """
    ...
    :param inversion: (optional) A boolean indicating whether the parsed value should be inverted (i.e., all values not in the parsed value will be returned), defaults to `False`.
    ...
    if value == TOKEN_JOKER:
        return parse_joker(valid_range, inversion)

    if value.isnumeric():
        return parse_numeric(value, valid_range, inversion)

    if value.isalpha():
        return parse_alias(value, valid_range, aliases, inversion)

    raise CroneeValueError(f"Invalid value '{value}'")


def parse_joker(valid_range: set, inversion: bool) -> set[int]:
    """
    Returns the valid range, if inversion is False, raise an error if inversion is True

    :param valid_range: A set of integers representing the valid range of values.
    :param inversion: A boolean indicating whether the parsed value should be inverted (i.e., all values not in the parsed value will be returned)
    :return: a copy of the valid_range set.
    :raises: CroneeSyntaxError, if the `inversion` is True
    """
    if inversion:
        raise CroneeSyntaxError(f"Joker cannot be inverted.")
    return valid_range.copy()


def parse_numeric(value: str, valid_range: set[int], inversion: bool) -> set[int]:
    """
    ...
    if inversion:
        return valid_range - {new_value}
    return {new_value}


def parse_alias(value: str, valid_range: set[int], aliases: dict[str, set[int]], inversion: bool) -> set[int]:
    """
